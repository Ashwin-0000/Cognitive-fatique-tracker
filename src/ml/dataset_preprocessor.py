...
class DatasetPreprocessor:
    ...
    def _synthesize_activity_from_nasa_tlx(self, row: pd.Series) -> np.ndarray:
        ...
        activity_rate_5min = activity_rate_1min * (0.9 + np.random.uniform(-0.1, 0.1))
        activity_rate_15min = activity_rate_5min * (0.85 + np.random.uniform(-0.1, 0.1))
        ...
        blink_rate_5min_avg = blink_rate * (0.95 + np.random.uniform(-0.05, 0.05))
    ...
    def balance_dataset(
        ...
        logger.info(f"Balanced dataset: {len(features)} → {len(balanced_features)} samples")
