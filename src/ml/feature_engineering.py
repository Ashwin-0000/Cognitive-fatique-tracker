...
class FeatureEngineer:
    ...
    def _extract_activity_features(
            self, current_time: datetime) -> Dict[str, float]:
        ...
        keyboard_count = self._count_events_by_type(
            current_time, 5, 'keyboard')
        mouse_count = self._count_events_by_type(
            current_time, 5, ['mouse_click', 'mouse_move', 'mouse_scroll'])
        ...
        features['activity_variance'] = np.std(
            minute_rates) if minute_rates else 0.0
        ...
        if self._baseline_activity_rate and self._baseline_activity_rate > 0:
            features['activity_decline_ratio'] = (
                activity_5min / 5.0) / self._baseline_activity_rate
        else:
            features['activity_decline_ratio'] = 1.0

        return features

    def _extract_eye_features(self,
                              current_time: datetime,
                              current_blink_rate: float) -> Dict[str,
                                                                 float]:
        ...
        features['blink_rate_5min_avg'] = np.mean(
            recent_blinks) if recent_blinks else current_blink_rate

        # Blink rate variance
        features['blink_rate_variance'] = np.std(
            recent_blinks) if len(recent_blinks) > 1 else 0.0
        ...
        if self._baseline_blink_rate and self._baseline_blink_rate > 0:
            features['blink_decline_ratio'] = current_blink_rate / \
                self._baseline_blink_rate
        else:
    ...
    def _extract_temporal_features(
        ...
        # Time of day category (0: morning, 0.33: afternoon, 0.66: evening, 1:
        # night)
        ...
        if self._session_start:
            elapsed_hours = (
                current_time - self._session_start).total_seconds() / 3600.0
            features['session_elapsed_normalized'] = min(
                elapsed_hours / 4.0, 1.0)
        else:
    ...
    def _extract_historical_features(
        ...
        score_5min_ago = self._get_score_at_time(
            current_time - timedelta(minutes=5))
        score_15min_ago = self._get_score_at_time(
        ...
        features['fatigue_avg_1hour'] = np.mean(
            recent_scores) if recent_scores else 0.0
        ...
        # Fatigue variance
        features['fatigue_variance'] = np.std(
            recent_scores) if len(recent_scores) > 1 else 0.0
