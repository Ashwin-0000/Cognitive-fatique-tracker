...
class PersonalizationEngine:
    ...
    def update_profile(
        ...
        self.profile['total_sessions'] = self.profile.get(
            'total_sessions', 0) + 1
    ...
    def _update_productive_hours(self, session_data: Dict):
        ...
        # EMA with alpha=0.2
        self.patterns['hourly_productivity'][str(
            hour)] = 0.8 * current + 0.2 * activity_score
            ...
            for i in range(min(bins, 8)):  # Max 8 bins (2 hours)
                ...
                if bin_scores:
                    ...
                    if len(self.patterns['fatigue_progression']['mean']) <= i:
                        self.patterns['fatigue_progression']['mean'].append(
                            mean)
                    else:
                        current_mean = self.patterns['fatigue_progression']['mean'][i]
                        self.patterns['fatigue_progression']['mean'][i] = 0.7 * \
                            current_mean + 0.3 * mean
    ...
    def get_feedback_summary(self) -> Dict:
        ...
        positive = sum(
            1 for f in self.feedback_history if f.get('type') == 'positive')
        dismissals = sum(1 for f in self.feedback_history if f.get(
