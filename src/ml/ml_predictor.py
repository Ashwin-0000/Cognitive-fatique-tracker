...
class MLPredictor:
    ...
    def predict(self, features: np.ndarray,
                use_ensemble: bool = True) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            if use_ensemble:
                ...
                predicted_score = np.average(
                    predictions, weights=weights_normalized)
                ...
                predicted_score = self.models['sgd'].predict(features_scaled)[
                    0]
                confidence = 0.5
            ...
            logger.debug(
                f"ML prediction: {predicted_score:.1f} (confidence: {confidence:.2f})")
        ...
        try:
            ...
            if not self._is_initialized:
                if len(self.training_buffer) >= 10:
                    ...
                    logger.debug(
                        f"Buffering samples: {len(self.training_buffer)}/10")
                    return
            ...
            logger.debug(
                f"Incremental update complete (sample #{self._training_samples_count})")
    ...
    def batch_train(
        ...
        logger.info(
            f"Batch training with {len(features_list)} samples from {dataset_source}")
        ...
        if len(self.training_buffer) > self.max_buffer_size:
            self.training_buffer = list(
                self.training_buffer[-self.max_buffer_size:])
        ...
        logger.info(
            f"Batch training complete: {self._training_samples_count} total samples")
    ...
    def train_from_psychometric_dataset(self, dataset_path: str) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Loaded {dataset.organization} {dataset.assessment_type} dataset")
            ...
            if 'nasatlx' in dataset.assessment_type.lower():
                ...
                raise ValueError(
                    f"Unsupported assessment type: {dataset.assessment_type}")
    ...
    def record_prediction_error(self, predicted: float, actual: float):
        ...
        if len(self.prediction_errors) >= 20:
            ...
            if recent_mae > 15.0:  # High error threshold
                logger.warning(
                    f"High prediction error detected (MAE: {recent_mae:.2f}), triggering retrain")
                self._full_retrain()
    ...
    def _initialize_models(self):
        ...
        logger.info(
            f"Initializing models with {len(self.training_buffer)} samples")
    ...
    def _full_retrain(self):
        ...
        logger.info(
            f"Performing full retrain with {len(self.training_buffer)} samples")
        ...
        for name in self.model_weights:
            if errors[name] != float('inf'):
                self.model_weights[name] = (
                    1.0 / (errors[name] + 1e-6)) / total_inverse
            else:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded model with {self._training_samples_count} training samples")
        except Exception as e:
