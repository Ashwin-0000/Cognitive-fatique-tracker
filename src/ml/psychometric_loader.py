...
class PsychometricLoader:
    ...
    def load_dataset(self, filepath: str) -> PsychometricDataset:
        ...
        organization, assessment, features = self._parse_filename(filepath.stem)
        
        logger.info(f"Loading {assessment.upper()} dataset from {organization}")
    ...
    def extract_nasa_tlx_features(self, dataset: PsychometricDataset) -> pd.DataFrame:
        """
        ...
        df['frustration_effort_ratio'] = df['frustration'] / (df['effort'] + 1e-6)
    ...
    def extract_cfq_features(self, dataset: PsychometricDataset) -> pd.DataFrame:
        """
        ...
        df['fatigue_ratio'] = df['physical_fatigue'] / (df['psychological_fatigue'] + 1e-6)
    ...
    def merge_datasets(self, datasets: List[PsychometricDataset]) -> pd.DataFrame:
        """
        ...
        logger.info(f"Merged {len(datasets)} datasets: {len(merged)} total samples")
    ...
    def get_statistics(self, dataset: PsychometricDataset) -> Dict:
        ...
        stats = {
            ...
            'fatigue_score': {
                'mean': float(df['fatigue_score'].mean()),
                'std': float(df['fatigue_score'].std()),
                'min': float(df['fatigue_score'].min()),
                'max': float(df['fatigue_score'].max())
            }
        }
