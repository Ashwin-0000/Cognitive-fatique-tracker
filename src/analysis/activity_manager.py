...
class ActivityManager:
    ...
    def track_activity_completion(
        ...
        if self.data_manager:
            try:
                ...
                print(
                    f"Warning: Could not save activity completion to database: {e}")
    ...
    def get_activity_stats(self) -> Dict:
        ...
        for comp in history:
            ...
            # Count
            activity_counts[activity_id] = activity_counts.get(
                activity_id, 0) + 1
        ...
        most_popular = max(activity_counts.items(), key=lambda x: x[1])[
            0] if activity_counts else None

        # Average fatigue reduction
        avg_reduction = sum(fatigue_reductions) / \
            len(fatigue_reductions) if fatigue_reductions else 0

        # Process category stats
        for cat in category_stats:
            reductions = category_stats[cat]['reductions']
            category_stats[cat]['avg_reduction'] = sum(
                reductions) / len(reductions) if reductions else 0
