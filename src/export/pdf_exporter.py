...
class PDFExporter:
    ...
    def export_report(
        ...
        try:
            ...
            # Title
            title = Paragraph("Cognitive Fatigue Tracker Report", self.title_style)
            story.append(title)
            ...
            if not sessions:
                story.append(Paragraph("No sessions found in the specified period.", self.styles['Normal']))
            else:
                # Summary statistics
    ...
    def _create_sessions_table(self, sessions: List) -> Table:
        ...
        table = Table(data, colWidths=[1.5*inch, 1*inch, 1*inch, 0.8*inch, 1*inch])
        table.setStyle(TableStyle([
