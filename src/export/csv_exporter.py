...
class CSVExporter:
    ...
    def export_statistics(
        ...
        try:
            ...
            total_duration = sum(s.get_stats()['total_duration_minutes'] for s in sessions)
            total_work = sum(s.get_stats()['work_duration_minutes'] for s in sessions)
            total_breaks = sum(s.get_stats()['break_count'] for s in sessions)
            total_activities = sum(s.get_stats()['total_activity_count'] for s in sessions)
            ...
            with open(output_file, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writerow(['Total Duration (hours)', f"{total_duration/60:.1f}"])
                writer.writerow(['Total Work Time (hours)', f"{total_work/60:.1f}"])
                writer.writerow(['Total Breaks', total_breaks])
                writer.writerow(['Total Activities', total_activities])
                writer.writerow(['Average Session Duration (minutes)', f"{total_duration/len(sessions):.1f}"])
                writer.writerow(['Average Breaks per Session', f"{total_breaks/len(sessions):.1f}"])
