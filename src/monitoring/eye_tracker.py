...
class EyeTracker:
    ...
    def start(self) -> bool:
        ...
        if not CV2_AVAILABLE:
            logger.error("OpenCV is not installed. Eye tracking requires opencv-python.")
            logger.info("Install with: pip install opencv-python")
            return False
        
        if not MEDIAPIPE_AVAILABLE:
            logger.error("MediaPipe is not installed. Eye tracking requires mediapipe.")
            logger.info("Install with: pip install mediapipe (requires Python 3.8-3.11)")
            return False
        
        if not NUMPY_AVAILABLE:
            logger.error("NumPy is not installed. Eye tracking requires numpy.")
        ...
        try:
            while self.is_running:
                try:
                    ...
                    logger.error(f"Error in capture loop iteration: {e}", exc_info=True)
                    time.sleep(0.1)
        except Exception as e:
    ...
    def _process_frame(self, frame):
        ...
        try:
            ...
            self._fps = 1.0 / (current_time - self._last_process_time) if self._last_process_time else 0
